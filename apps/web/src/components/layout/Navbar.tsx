...
export function Navbar() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-white/[0.065] bg-[#071018]/82 backdrop-blur-xl">
      ...
      >
        <Link to="/" className="group flex min-h-11 min-w-0 items-center gap-2.5 sm:gap-3" aria-label={`${profile.name} home`}>
          <span className="relative grid h-9 w-9 shrink-0 place-items-center overflow-hidden font-mono text-xs font-semibold text-cyan-100">
          ...
          <span className="min-w-0">
            <span className="block truncate text-xs font-semibold tracking-[-0.02em] text-white sm:text-sm">{profile.name}</span>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            className="grid h-11 w-11 place-items-center rounded-xl text-slate-300 transition hover:bg-white/[0.05] hover:text-white md:hidden"
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="mx-auto max-w-7xl border-t border-white/[0.055] bg-[#071018]/96 px-3 py-2 shadow-[0_24px_50px_rgba(0,0,0,.32)] md:hidden"
          >
            {[{ label: "Home", to: "/" }, ...links, { label: "Contact", to: "/contact" }].map((link) => (
              <NavLink
                ...
                to={link.to}
                className={({ isActive }) =>
                  `flex min-h-12 items-center justify-between border-b border-white/[0.045] px-3 py-3 text-sm font-medium transition last:border-b-0 ${isActive ? "text-cyan-100" : "text-slate-500 hover:text-white"}`
                }
              >
                {link.label}
